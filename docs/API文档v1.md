...
调整音乐库歌曲列表中的显示顺序，用于支持前端拖拽排序。

```
POST /library/songs/reorder?from={from}&to={to}
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| from | int | 是 | 原位置索引，从 0 开始 |
| to | int | 是 | 目标位置索引，从 0 开始 |
...
**错误情况：**
- from 或 to 超出范围：`{"success": false, "msg": "索引超出范围"}`

**说明：**
- 该接口只调整歌曲在列表中的展示顺序，不影响音频文件本身。
- 前端拖拽排序时调用，排序结果需要持久化到数据库，刷新页面后保持一致。
- 示例：`POST /library/songs/reorder?from=3&to=1` 表示将第 4 首歌移动到第 2 个位置。
